...
**関連ドキュメント**: `docs/design/session-operation-flow.md`（セッション外部の運用フロー設計 — ディスパッチャー、5シナリオ、ライフサイクル、横断的関心事）
...
`session-operation-flow.md` の設計で以下の不足ロールが特定されている。現時点では既存ロール + dev_manager で補完するが、不足が顕著になった場合に新設を検討する。

| 不足ロール | 関連シナリオ | 補完方法 |
|-----------|------------|---------|
| planner / architect | シナリオ1（要件定義〜設計） | dev_manager が要件分析を補完、documentarian が設計ドキュメント作成 |
| tester / eval_manager | シナリオ3（独立評価） | reviewer が独立レビューを担当、dev_manager が評価計画を補完 |
...
| 位置づけ | コード開発セッションの最上位オーケストレーター。ユーザーから開発タスクを受け取り、各開発ロールを起動する |
...
2. **タスク分配**: ユーザーから受けた開発タスクを分析し、適切なロールへの細分化タスク（DT-XXX）を作成・配分する
...
8. **完了報告**: 全ロールの作業が完了したら、ユーザーに開発タスクの完了を報告する

#### 5.2.3 作業フロー

```
1. ディスパッチャーからのセッション起動コンテキストを受け取る（シナリオ判定結果・目的・対象・制約・品質要件を含む）
...
9. 全ロール完了後、report.md を作成し、ユーザーに開発タスク完了を報告する
```
...
#### 5.3.3 セッション間引き継ぎプロトコルの追記案

`session-operation-flow.md` §11 で定義されたセッション間引き継ぎプロトコルを common.md に反映する。以下の情報を common.md の新規 §7 として追加する。

- **コンテキスト分離の原則**: 新セッションは前セッションの内部状態を引き継がない。引き渡すのは成果物とドキュメントのみ
- **ファイルパス参照方式**: 引き継ぎ情報は原則としてファイルパスで参照する
- **git コミットによる成果物共有**: コード変更はすべて git コミットとして共有する
- **エラー回復5パターンへの参照**: セッション中断時の復旧方法（`session-flow-advanced.md` §11.3）への参照
