...
| **完了報告ファイル** | セッション間で参照可能・人間可読 | 開発セッション→評価セッションの引き渡し、施策管理 L2 への報告 |
...
セッション間のコンテキスト引き継ぎは以下の3パターンに分類される。
...
| 3 | **施策管理との接続** | 開発フローの成果を施策管理レイヤーに報告する | 開発セッション完了 → L2-worker が `03_work_log.md` に記録 |
...
graph LR
    subgraph "開発フローレイヤー"
        Session["開発セッション完了"]
        Report["完了報告<br/>（成果物一覧・テスト結果・品質指標）"]
        Session --> Report
    end

    subgraph "施策管理レイヤー"
        L2W["L2-worker"]
        WorkLog["03_work_log.md"]
        L2W --> WorkLog
    end

    Report -->|"L2-worker が人間として<br/>開発フローを利用した場合"| L2W
```

| 引き渡し情報 | 形式 | 記録先 |
|-------------|------|--------|
| セッション完了報告 | 成果物一覧・テスト結果・品質指標 | `03_work_log.md` の作業内容として L2-worker が記録 |
| git コミット | コミットハッシュ・変更ファイル一覧 | `03_work_log.md` の成果物として参照 |
| 発見した課題 | 課題の概要と対応提案 | `07_issues.md` に L2-worker が起票 |
...
4. **人間を介した引き継ぎ**: シナリオ間の引き継ぎは人間（またはL2-worker）がディスパッチャーへの入力として情報を整理する。自動的なセッション連鎖は行わない
...
| **回復手順** | 1. dev_manager がブロック報告を分析する<br/>2. **解決可能な場合**: タスク指示を修正して同一ロールに再指示、または別ロールにタスクを振り替える<br/>3. **解決不可能な場合**: Level 2 エスカレーション（L1 または人間に報告）<br/>4. 人間が追加情報・方針変更を提供した後、再計画して再開 |
...
| `dev-workflow-overview.md` | §2.1 ロール構成と階層 | §3.3 セッション型レジストリ | 上位レイヤーとして補完 | 既存のロール構成図は L1 → dev_manager → 7ロールの階層。本設計では人間→ディスパッチャー→ dev_manager に変更（§4.1.2）。**既存図の L1 → dev_manager の矢印の注釈を更新する必要がある**（§4.4 の責務分界に従い、直接起動ではなく L2-worker 経由に変更） |
...
| `dev-workflow-overview.md` | §3.1 施策管理との比較表 | §4.4 施策管理と開発フローの責務分界 | **変更が必要** | 既存の比較表では「マネージャー: L1」「L1 → dev_manager: 開発タスク発行」と記載。本設計では人間（またはL2-worker）が直接エントリーするため、**§3 の連携ポイントの記述を更新する必要がある** |
| `dev-workflow-overview.md` | §3.2 連携ポイント | §4.4.3 施策管理から開発フローを利用するケース | **変更が必要** | 既存のシーケンス図で L1 → dev_manager の直接タスク発行が描かれている。本設計では L2-worker が「人間」として開発フローを利用する間接接続に変更。**シーケンス図の更新が必要** |
...
| `dev-workflow-detail.md` | §4.3 エスカレーション手順 | §11.3 エラー回復 | 上位レイヤーとして補完 | 既存のエスカレーション手順（Level 1〜3）は変更なし。本設計では5つのエラーパターンと回復手順を追加定義。エスカレーション先として「L1」に加えて「人間」を明記（§4.4 の責務分界に従う） |
| `dev-workflow-detail.md` | §4.4 コンテキスト共有 | §11.2 コンテキスト引き継ぎ | 上位レイヤーとして補完 | 既存のコンテキスト共有（タスク指示・git コミット・ファイルパス参照・完了報告の申し送り）は変更なし。本設計ではセッション間の引き継ぎ（同一シナリオ継続・異なるシナリオ・施策管理接続）を追加定義 |
| `dev-workflow-detail.md` | §5.1 各ロール修正提案 | 直接対応なし（後続施策の対象） | 追加が必要 | 既存の修正提案（8ロール分）は後続施策「ロール定義の実装」で実施する。本設計ではシナリオ1・シナリオ3 での不足ロール（planner, architect, tester, eval_manager）を新たに特定しており、この情報も修正提案に追加する必要がある |
| `dev-workflow-detail.md` | §5.2 dev_manager 要件定義 | §4.4.4 dev_manager の内部責務（不変部分の確認） | **変更が必要** | 既存の dev_manager 要件定義では「L1 からの開発タスクを受領する」「L1 に完了報告する」と記載。本設計では起動トリガー源と完了報告先が変更されるため、**§5.2.3 の作業フローと §5.2.5 のやらないことを更新する必要がある** |
| `dev-workflow-detail.md` | §5.3 common.md 修正提案 | 直接対応なし（後続施策の対象） | 追加が必要 | 既存の修正提案は変更なし。本設計で追加された情報（セッション間引き継ぎプロトコル、エラー回復手順）を common.md に反映する修正提案を追加すべき |
| `dev-workflow-detail.md` | §6 まとめ・引き渡し | §13 まとめと次ステップ | 上位レイヤーとして補完 | 既存のまとめで列挙された後続施策は本設計の§13 に引き継ぐ |

### 12.3 統合マッピングのサマリ

| 変更の種類 | 件数 | 説明 |
|-----------|------|------|
| 上位レイヤーとして補完 | 11件 | 本設計が既存設計のセッション外部レイヤーとして機能。既存設計の変更は不要で、参照リンクの追加を推奨 |
| 整合性確認済み | 7件 | 既存設計と本設計の間に矛盾がないことを確認。変更不要 |
| 変更が必要 | 3件 | L1→dev_manager の直接接続を人間→ディスパッチャー→dev_manager に変更する部分。既存ドキュメントの更新が必要 |
| 追加が必要 | 2件 | ロール修正提案への不足ロール情報追加、common.md 修正提案へのセッション間プロトコル追加 |

### 12.4 変更が必要な箇所の詳細

後続施策「ロール定義の実装」で対応すべき変更を優先度順に列挙する。

| # | 対象ドキュメント | 変更箇所 | 変更内容 | 優先度 |
|---|---------------|---------|---------|--------|
| 1 | `dev-workflow-overview.md` §3.2 | 連携ポイントのシーケンス図 | L1 → dev_manager の直接タスク発行を、L2-worker が「人間」として開発フローを利用する間接接続に変更 | 高 |
| 2 | `dev-workflow-overview.md` §3.1 | 施策管理との比較表 | dev_manager への入力元を L1 から人間/ディスパッチャーに変更 | 高 |
| 3 | `dev-workflow-detail.md` §5.2.3 | dev_manager の作業フロー | ステップ1「L1 からの開発タスクを受領する」を「ディスパッチャーからのセッション起動を受け取る」に変更 | 高 |
| 4 | `dev-workflow-overview.md` §2.1 | ロール構成と階層の図 | L1 → dev_manager の矢印の注釈を更新（直接起動 → L2-worker 経由の間接接続） | 中 |
...
本設計は「設計」であり、実際のロール定義ファイルの作成・修正は後続施策で行う。

#### 13.3.1 実装すべき内容

| # | 実装項目 | 参照元 | 優先度 |
|---|---------|--------|--------|
| 1 | `roles/dev_manager.md` の新規作成 | `dev-workflow-detail.md` §5.2 + 本設計 §4.4.4 の変更点 | 高 |
...
| 2 | ディスパッチャーの実装形態が未確定（Claude Code のスキル・サブエージェント・CLAUDE.md のいずれで実装するか） | §3 | 後続施策で実装方式を決定する |
| 3 | セッション間の自動連鎖は行わない設計（人間を介した引き継ぎ）| §11.2.3 | 運用の成熟に伴い、自動連鎖の導入を検討する余地あり |
| 4 | openspec 連携が未定義（ISS-006） | `dev-workflow-overview.md` §5.4 | openspec 開発ライフサイクルが定義された後に統合施策を実施 |

---

**作成者**: L2-worker
**作成日**: 2026-03-01
**関連施策**: シナリオ別セッション運用フロー設計（Phase 1, Phase 2, Phase 3）
