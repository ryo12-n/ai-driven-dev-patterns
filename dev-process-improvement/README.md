...
```
「あなたはトリアージセッションです。dev-process-improvement/.claude/rules/triage-manager.mdに従ってください。」
```
...
| Claude 常時参照 | `.claude/rules/` | 毎リクエストで自動読み込み。セッションルール（l1-manager, l2-worker 等） |
| Claude オンデマンド参照 | ルート側 `.claude/skills/` | 特定タスク実行時のみ読み込み。トリアージポリシー等 |

判断基準: 「毎回のリクエストで参照が必要か？」→ Yes なら `rules/`、No なら `skills/`。
詳細はルートの [CLAUDE.md](../CLAUDE.md) の「文書分類ポリシー」セクションを参照。
