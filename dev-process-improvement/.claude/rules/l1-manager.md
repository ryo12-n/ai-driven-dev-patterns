...
3. **バックログファイルの削除確認**: 施策開始時に `backlog/<施策名>.md` が削除されていることを確認する（T-003 のルールに従い施策開始時に削除済みのはず）
4. **アーカイブ移動**: 施策ディレクトリを `_archive/` に移動する

```bash
git mv initiatives/<施策名>/ initiatives/_archive/<施策名>/
```

5. 移動後、コミットしてクローズ完了とする

> チェックリスト1〜3 に不備がある場合は、不備を修正してからアーカイブ移動を行う。
