...
- PR のタイトルは `triage: YYYYMMDD トリアージレポート` の形式にする（日付はフォルダ名・コミットカテゴリと同じ compact 形式）
...
- **backlog 関係性分析（TG-005）**: backlog 内のアイテム間の関係性を横断的に分析し、以下の3種類の関係を検出する
  - **重複候補**: 課題・背景が類似している / 共通の成果物を持つ / 同一ファイルを変更対象としている
  - **依存関係**: 一方が他方の前提条件になっている / 一方の成果物が他方の入力となる
  - **統合候補**: 重複候補のうち1施策にまとめた方が効率的 / 個別に実施すると整合性確保のコストが高い
  - 分析結果は `03_report.md` の「backlog 関係性分析（TG-005）」テーブルに記載する
  - 統合・順序変更などの提案はレポートに記載し、実行はユーザー判断に委ねる
- **backlog 施策化済みチェック（TG-002 拡張）**: TG-002 の棚卸し時に、各 backlog ファイルが既に施策化されていないかを確認する
  - 各 backlog ファイルに対し `initiatives/`（活動中）および `initiatives/_archive/`（完了済み）と突合する
  - 施策化済みのファイルを `03_report.md` の「削除・クローズ候補」セクションにレポート記載する
  - 削除はユーザーの許可を得てから実施する（勝手に削除しない）
