...
| **知見集約ルーティング** | 横展開テーブル、ルーティング先（CSV/backlog/inbox/なし）の明記 | 施策の知見を組織に還元する |
...
> **現状の不適合**: triage-evaluator には壁打ちフェーズと知見記録セクションが明示されていない。今後のルール改訂で追加を検討すること。
...
| フォーマット不備（停止ルール・担当ファイル等の欠落） | `プロセス改善_課題管理.csv` に起票（優先度: 低） |
