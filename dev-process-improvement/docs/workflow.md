...
[L1 マネージャー]
  ...
  施策完了（全フェーズ通過）時:
    → initiative クローズ手順を実施
      ...
      4. backlog ファイルの削除確認
      5. git mv initiatives/<施策名>/ initiatives/_archive/<施策名>/
      6. コミット・push → PR 作成
```
...
3. ワーカーセット作成・ディスパッチ
                   3a. triage-worker をサブエージェントとして起動
                       ...
                       ・セッション構造標準ポリシーチェック（TG-008）:
                         ...
                         `.claude/rules/triage-standard-policy-guideline.md` の基準と突合し、
...
6. レポート作成  → triage/YYYYMMDD/03_report.md を作成 → PR として提出
7. ユーザーレビュー
8. backlog 反映  → 承認後に backlog/ へ個別ファイルを作成（`_template.md` をコピー）
9. 後処理        → 処理済み inbox アイテムを削除（git 履歴が証跡）
10. 課題起票     → 課題と確定したものを プロセス改善_課題管理.csv に直接起票
```
