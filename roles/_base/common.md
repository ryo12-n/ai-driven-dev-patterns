...
ユーザー（人間）
  │
  └── dev_manager（開発タスクのオーケストレーター・シナリオ判定）
        │
        ├── 専門ロール A: 順次または並列に起動
        ├── 専門ロール B
        └── 専門ロール C
...
コード開発タスクでは、dev_manager がユーザーの意図を5シナリオに分類し、各ロールの間に位置する。

```
ユーザー（人間）
  │
  └── dev_manager（シナリオ判定・開発マネージャー）
        │
...
1. **ルール化候補**（`.claude/rules/` や `roles/` に反映できるパターン）
