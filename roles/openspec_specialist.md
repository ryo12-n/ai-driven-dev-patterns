...
1. スキルを実行する
2. スキルの実行中にユーザー入力が必要な場合は、Q&A シートを通じて dev_manager と連携する（下記「壁打ち連携プロトコル」参照）
3. スキル実行結果を記録する
...
- 質問が発生した時点で作業を中断し、dev_manager にブロック報告する
- dev_manager がユーザーとの壁打ちを行い、Q&A シートに回答を記入する
- 回答が揃ったら作業を再開する
...
報告に含める内容:
- 実行したスキルとその結果
- 作成・更新した OpenSpec アーティファクト
- 仕様上の判断事項とその根拠
- Q&A シートの概要（壁打ちが発生した場合）
- 後続タスクの推奨（実装が必要な場合の委譲先ロール等）
