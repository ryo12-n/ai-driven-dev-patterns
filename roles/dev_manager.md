...
- タスクの目的・スコープ・不明点を plan.md の「壁打ち」セクションに記録する
  - **理解サマリー**: タスクの目的・完了条件を自分の言葉でまとめる
  - **スコープ**: 対象ファイル・モジュール・ドキュメントを明示する
  - **不明点の洗い出しと解消**: 着手前の不明点を列挙し、ディスパッチャー経由でユーザーに確認する
...
- 計画策定前に関連する設計書・仕様書を確認する（**必須**）
  - `docs/design/` 配下の関連設計書
  - `openspec/specs/` 配下の関連仕様書
  - 前セッションの成果物（`sessions/` 配下の関連セッション）
- 確認した文書とその影響を plan.md の「参照情報評価」セクションに記録する
- 参照結果から得られた計画への制約・影響をまとめる

#### 3.3 計画策定

- TDD分離判断を行う（シナリオ2・新機能の場合、`dev-workflow-detail.md` §2 の基準に基づく）
- ロール起動計画を策定する（起動順序・各ロールへのタスク指示の概要）
- plan.md の「前提条件」「やること」「やらないこと」「完了条件」「リスク」を記入する

#### 3.4 計画承認ゲート（必須）

- plan.md を完成させた後、**ディスパッチャー経由でユーザーに計画を提示し、承認を得る**
- ユーザーの承認を得るまで §4 ロール起動に進んではならない
- ユーザーから修正指示があった場合は plan.md を修正し、再度承認を求める
- 承認結果を log.md に記録する（承認日時・修正があった場合はその内容）
...
- 関連する設計書・仕様書の事前確認（参照情報評価）と plan.md への記録
- 計画のユーザー承認取得（承認を得るまでロール起動に進まない）
